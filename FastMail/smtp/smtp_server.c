...
void store_email(const char *sender_email, const char *recipient_email, const char *subject, const char *email_content){
    if (!strstr(recipient_email, "@fastmail.com")) {
        printf("❌ Email not for @fastmail.com, ignoring.\n");
        return;
    }
    ...
    char *content = PQescapeLiteral(conn, email_content, strlen(email_content));
    ...
    snprintf(query, sizeof(query),
             ...
             "VALUES (%s, %s, %s, %s, NOW(), false);", s_id, r_id, subj, content);
    ...
    PQclear(res);
    PQfreemem(s_id); PQfreemem(r_id); PQfreemem(content); PQfreemem(subj);
    free(sender_id); free(recipient_id);
}

void store_sent_email(const char *sender_email, const char *recipient_email, const char *subject, const char *email_content){
    char *sender_id = get_or_create_user(sender_email);
    char *recipient_id = get_or_create_user(recipient_email);
    if (!sender_id || !recipient_id) {
        fprintf(stderr, "❌ Could not get or create user IDs for sent mail.\n");
        free(sender_id); free(recipient_id);
        return;
    }
    ...
    PGresult *res = PQexec(conn, query);
    if (PQresultStatus(res) != PGRES_COMMAND_OK)
        fprintf(stderr, "❌ Failed to log sent email: %s\n", PQerrorMessage(conn));
    else
        printf("📨 Sent email logged locally.\n");
...
int forward_email_to_mailbox(const char *sender, const char *recipient, const char *subject, const char *message) {
    int sock = socket(AF_INET, SOCK_STREAM, 0);
    if (sock < 0) return -1;

    struct sockaddr_in addr = {0};
    addr.sin_family = AF_INET;
    addr.sin_port = htons(2525);  // MailBox port
...
void *handle_client(void *arg) {
    ...
    send(client_fd, "220 FastMail SMTP Ready\r\n", 25, 0);
    ...
    while (1) {
        ...
        if (strncasecmp(buffer, "MAIL FROM:", 10) == 0) {
            ...
            if (!subject || !body) {
                ...
                forward_email_to_mailbox(sender, recipient, subject, body);
                store_sent_email(sender, recipient, subject, body);
                send(client_fd, "250 Email forwarded to MailBox\r\n", 32, 0);
            } else if (strstr(recipient, "@fastmail.com")) {
                store_email(sender, recipient, subject, body);
                send(client_fd, "250 Email stored in FastMail\r\n", 31, 0);
            } else {
...
int main() {
    char *dbname = getenv("FASTMAIL_DBNAME");
    char *user = getenv("FASTMAIL_DBUSER");
    char *pass = getenv("FASTMAIL_DBPASSWORD");
    char *host = getenv("FASTMAIL_DBHOST");
    char *port = getenv("FASTMAIL_DBPORT");
